...
enum TicketNewError {
    TitleInvalid { explanation: String },
    DescriptionInvalid { explanation: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleInvalid { explanation }) => panic!("{}", explanation),
        Err(TicketNewError::DescriptionInvalid { explanation }) => {
            Ticket {
                title: title,
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleInvalid {
                explanation: "Title cannot be empty".to_string(),
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleInvalid {
                explanation: "Title cannot be longer than 50 characters".to_string()
             });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionInvalid { explanation: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionInvalid { explanation: "Description cannot be longer than 500 characters".to_string() })
        }
